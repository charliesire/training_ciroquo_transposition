...
def MCMC_alpha_multichains(index_calib, scale, num_chain, tune_size, size, M, alpha_min, alpha_max, delta_alpha, rngseed, results_measures, sigma, mm_list, index_lambda_p, index_lambda_q, bMINlambda, bMAXlambda, pre_path, loo = True):
    ...
    for idx_loo in list_idx_loo:
        ...
        Ysimu_list, Ystd_list, stored_likelihoods = get_likelihoods_dflambda(df_Lambda = df_Lambda.values, sigma = sigma, mm_list = mm_list, results_measures = results_measures, index=[index_calib], std_code = True, idx_loo = idx_loo) #get likelihoods
