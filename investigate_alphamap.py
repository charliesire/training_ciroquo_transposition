...
def func_alphamap(index_calib, M, iter_lim, threshold, alpha_min, alpha_max, delta_alpha, scale, results_measures, sigma, myCODE, mm_list, index_lambda_p, index_lambda_q, bMINlambda, bMAXlambda, pre_path, loo = True):
    ...
    for idx_loo in list_idx_loo: #for each observation x_j
        ...
        while iter <= iter_lim and norm2(alpha_new,alpha_star)> threshold: #while not fixed point
            ...
            Ysimu_list, Ystd_list, stored_likelihoods = get_likelihoods_dflambda(df_Lambda = df_Lambda.values, sigma = sigma, mm_list = mm_list, results_measures = results_measures, index=[index_calib], std_code = True, idx_loo = idx_loo) #compute likelihood of each lambda
